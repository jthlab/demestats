...
class SingleDeme:
    ...
    class Exponential:
        ...
        def __init__(self, size=1.0, t=1.0, g=1.0):
            ...
            model1 = momi2.DemographicModel(N_e=size, muts_per_gen=1)
            model1.add_leaf("A", g=g, N=size_end)
...
class TwoDemes:
    class Constant:
        ...
        def migration_twophase(self, tstart=1.0, tend=0.0, rate1=0.05, rate2=0.01):
            model1 = None
            b = self.b
            b.add_migration(
                source="B", dest="A", start_time=tstart / 2, end_time=tend, rate=rate1
            )
            b.add_migration(
                source="B", dest="A", start_time=tstart, end_time=tstart / 2, rate=rate2
            )
            g = b.resolve()
            return g, model1
