...
def _downsample(n: Int[ScalarLike, ""]) -> sps.sparray:
    ...
    """
    i = np.arange(1, n + 1)
    return sps.diags_array([n - i + 1, i], offsets=[0, 1], shape=(n, n + 1)) / n


def _drift(n: Int[ScalarLike, ""]) -> sps.sparray:
...
def _migration(ns):
    ...
    Phi3 = sps.diags_array(np.arange(nk + 1))
    # the last term is (ik + 1) * Phi[n](i+ek) = (ik + 1) * Sek
    Phi4 = sps.diags_array(np.arange(1, nk + 1), offsets=1)
    ret2 = {1: Phi4 - Phi3}
    return ret1, ret2


def _mutation(n: Int[ScalarLike, ""]) -> sps.sparray:
    "mutation operator"
    i = np.arange(n + 1)
    return sps.diags_array(
        [(n - i + 1)[1:], -n, (i + 1)[:-1]], offsets=[-1, 0, 1], shape=(n + 1, n + 1)
    )


def _ibis(n: Int[ScalarLike, ""]):
...
def _jackknife0(n: Int[ScalarLike, ""]) -> sps.sparray:
    ...
    return sps.coo_array((data, (row, col)), shape=(n, n - 1))


def _jackknife(n: Int[ScalarLike, ""]) -> sps.sparray:
