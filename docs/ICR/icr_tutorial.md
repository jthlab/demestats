...
`demestats` package (specifically the `demestats.iicr` modules). `demestats` also includes
...
curve `log_s(t)`. While implementing `demestats`, this concept was originally introduced as the IICR described by Mazet et al. (2016) 
and is the reciprocal of that hazard:

`IICR(t) = 1 / c(t)`. 

However, the inference pipeline in `demestats` relies mostly on `c(t)` hence why we stick with using the term ICR. 
...
The `IICRCurve` and `IICRMeanFieldCurve` objects are the core components. First construct the objects by passig in a `demes.Graph` and a sample size `k`.
Then you can use either to map a set of *time points* and *sampling configuration* to the expected ICR curve under a demographic model. 
...
iicr_exact = IICRCurve(demo=g, k=2)
expected_exact = iicr_exact(params={}, t=t, num_samples={"P0": 1, "P1": 1})
# you can also use a one liner IICRCurve(demo=g, k=2)(params={}, t=t, num_samples={"P0": 1, "P1": 1})

iicr_meanfield = IICRMeanFieldCurve(demo=g, k=2)
expected_meanfield = iicr_meanfield(params={}, t=t, num_samples={"P0": 2, "P1": 0})
```

Note that passing in `params={}` evaluates the expected ICR under the constructed demographic model `g`. The sampling configuration must add up to the initial `k` used
...
The `params` dict can then be passed into `IICRCurve` and `IICRMeanFieldCurve`:

```python
iicr_exact = IICRCurve(demo=g, k=2)
expected_exact = iicr_exact(params=params, t=t, num_samples={"P0": 1, "P1": 1})

iicr_meanfield = IICRMeanFieldCurve(demo=g, k=2)
expected_meanfield = iicr_meanfield(params=params, t=t, num_samples={"P0": 2, "P1": 0})
```

## IICR log-likelihood

For likelihood-based inference, use the ICR log-likelihood helper from
`demestats.fit.fit_mrpast`.
...
an array of integers `sample_config` and an array of strings `deme_names` for the population names. For example, if one wants to use `{"P0": 0, "P1": 2}` then
 `sample_config` would be [0, 2]. Note that `iicr_call` requires an `IICRCurve` or `IICRMeanFieldCurve` object.
...
3. Construct `IICRCurve` or `IICRMeanFieldCurve` object and obtain the expected ICR.
