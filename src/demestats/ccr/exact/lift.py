...
def _lift_expm(
    ...
    *,
    ...
    # expm_unif's vector path uses a shared step count, which can introduce
    # discontinuities when the step count changes across t. Evaluate per t.
    res = jax.vmap(lambda dti: expm_multiply(Q, dti, vec0))(dt)
    res = jnp.asarray(res)
    p_nc = res.sum(axis=1)
