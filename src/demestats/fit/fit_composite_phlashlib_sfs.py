...
def _compute_composite_phlashlib_sfs_likelihood(
    vec,
    path_order,
    esfs,
    proj_dict,
    ...
    arg_loss = _compute_phlashlib_likelihood(
        vec,
        path_order,
        times,
        deme_names,
    ...
    sfs_loss = _compute_sfs_likelihood(
        vec,
        path_order,
        esfs,
        proj_dict,
...
def fit(
    ...
    *,
    ...
    L, LinvT = make_whitening_from_hessian(
        _compute_composite_phlashlib_sfs_likelihood, x0, *args
    )
    g = pullback_objective(
        _compute_composite_phlashlib_sfs_likelihood, x0, LinvT, *args
    )
    y0 = np.zeros_like(x0)
