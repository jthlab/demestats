...
from .mf import CCRMeanFieldCurve
...
class CCRCurve:
    ...
    def _setup_aux(
        ...
        return traverse(
            ...
            aux=None,
            scan_over_lifts=False,  # CCR aux contains scipy sparse matrices.
        )[1]
    ...
    def __call__(
        ...
        max_vmap = int(os.environ.get("DEMESINFER_CCR_VMAP_MAX_T", "32"))
    ...
    def curve(
        ...
        max_states = int(os.environ.get("DEMESINFER_CCR_MAX_STATES", "1000000"))
        ...
        f = _call(et, self.k, demo, dict(num_samples), self._setup_aux(et))
...
def _call(
    ...
    _, auxs = traverse(
        et, states, node_callback, lift_callback, aux=aux, scan_over_lifts=False
    )
    interps = [d["interp"] for d in auxs.values() if "interp" in d]
    return MergedInterp(interps)


__all__ = ["CCRCurve", "CCRMeanFieldCurve", "lift", "events", "interp"]
