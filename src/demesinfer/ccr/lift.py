...
from demesinfer.numba.lift_nd_mats import mats_ccr
...
def _lift_expm(
    ...
    *,
    ...
    grid_n = int(os.environ.get("DEMESINFER_CCR_EXPM_GRID", "128"))
    max_entries = int(os.environ.get("DEMESINFER_CCR_EXPM_MAX_ENTRIES", "20000000"))
...
def execute(
    ...
    if constant:
        ...
        if np.isinf(float(t1)):
            ...
            tmax_env = os.environ.get("DEMESINFER_CCR_TERMINAL_TMAX", "")
            if tmax_env:
                tmax = float(tmax_env)
            else:
