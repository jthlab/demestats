...
def zarr2het_specific_comparison(
    ds: xr.Dataset, samples: list[tuple[str, str]], w: int = 100
):
    """
...
def compile(ts, subkey, a=None, b=None):
    ...
    if a is None and b is None:
        samples = jax.random.choice(
            subkey, ts.num_samples / 2, shape=(2,), replace=False
        )
...
def get_het_data_from_ts(
    ts, num_samples=100, option="random", seed=2, window_size=100, mask=None
):
    key = jr.PRNGKey(seed)
    cfg_list = []
    all_config = []
    ...
    result = (
        zarr2het_specific_comparison(ds, names[all_config], 100).compute().todense()
    )
